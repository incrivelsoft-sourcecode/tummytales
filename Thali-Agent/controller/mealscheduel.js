...
function getNextReminderTime() {
  const next = new Date();
  next.setDate(next.getDate() + 1); // adds 1 day
  return next.toISOString(); // returns next day's ISO string
}
...
cron.schedule("* * * * *", async () => {
    ...
    try {
      ...
      for (const meal of pendingMeals) {
        ...
        meal.aiGeneratedMeal = parsedMeal; // overwrite even if previous exists
        meal.reminderTime = getNextReminderTime(); // push reminder to next day
        await meal.save();
